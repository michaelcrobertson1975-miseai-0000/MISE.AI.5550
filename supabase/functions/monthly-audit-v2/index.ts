// @ts-nocheck
/**
 * monthly-audit-v2 - backs the app's Recap tab (the "MONTHLY AUDIT" TODO
 * block at the top of the HTML). Uses the SAME Gemini/Vertex credential
 * this project already has configured for invoice reading (ingest-invoice)
 * -- no new API key needed.
 *
 * (Named -v2: the plain monthly-audit slug got stuck serving a stale
 * pre-fix response after its first deployment and never recovered even
 * after the code was corrected -- same platform quirk hit on prep-sync,
 * abandoned rather than fought further.)
 *
 * DATA IN: real Postgres only -- completed invoices/lines for the period,
 * price-move outliers (same source as Price Moves), pnl_periods +
 * pnl_manual_costs if the owner has entered sales/labor for that period,
 * nightly_reports as a fallback sales/labor signal, and prep_yield_log
 * activity. Whatever is missing is reported to the model as missing, with
 * an explicit instruction not to invent numbers to fill a gap. Verified
 * live against Cellar (Sept 2026): correctly reported zero data rather
 * than fabricating a narrative when nothing qualified yet.
 *
 * STORAGE: upserts public.monthly_audits keyed by (client_id, period) so
 * re-running the same month overwrites rather than duplicates.
 *
 * AUTH: restaurant_id + api_token must match clients.api_token.
 *
 *   POST /monthly-audit-v2  { restaurant_id, api_token, year, month }
 *     -> { success, period, narrative, highlights, model_used, data_used }
 *   GET  /monthly-audit-v2?restaurant_id=&api_token=&year=&month=
 *     -> returns a previously generated audit for that period if one
 *        exists, without calling the model again.
 */
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.error('[monthly-audit-v2] unhandled:', err?.message ?? err);
